...
namespace Assets.PolyTycoon.Scripts.Transportation.Model.TransportRoute
{
	public class TransportRouteManager : MonoBehaviour 
	{
		...
		public void OnTransportRouteCreate()
		{
			...
			{
				Vehicle = RouteCreateController.RouteVehicleChoiceController.SelectedVehicle,
				RouteName = RouteCreateController.RouteElementController.RouteNameField.text
			};
			foreach (RouteElementView view in RouteCreateController.RouteElementController.TransportRouteElementViews)
			{
				transportRoute.TransportRouteElements.Add(view.TransportRouteElement);
			}
